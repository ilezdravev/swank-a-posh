...
class ProductCard {
  ...
  showQuickShop() {
    ...
    var _this = this;
    $('body').on('click', '.js-quick-shop-link, .quick-shop__popup .color-swatch-container', e => {
      e.preventDefault();
      const $currentTarget = $(e.currentTarget);
      _this.buildQuickShop($currentTarget);
      ...
      // console.log($currentTarget,$currentTarget.data('url'));
      var html = window.localStorage.getItem('products_data');
      html = JSON.parse(html);
      var url = $currentTarget.data('url');
      if(window.localStorage.getItem('products_data') && html[url]){
        $('.js-quick-shop .product_form').html(html[url]);
      }else{
      window.PXUTheme.jsProductClass.load($currentTarget.data('url')).then(({
        html
      }) => {
        $('.js-quick-shop .product_form').html($(html.content).find('.product_form').html());
      }).catch(error => console.error(e));
    }

    });
  }
  buildQuickShop(target){
    const container = target.closest('.gallery-cell').find('.color-swatch-container .selected').closest('.color-swatch-container');
    
    const data = JSON.parse(container.find('script').html());

    const quickShop = $("#quickshop");
    ...
    quickShop.find('.fs-images-bar-container').html(image_html);

    quickShop.find('a.quick-view-dialog-image').attr('href',`/products/${data.handle}`);
    
    const new_image = images[0].split('.jpg')[0] + '_800x.jpg' + images[0].split('.jpg')[1];

    quickShop.find('a.quick-view-dialog-image').html(`<img loading="lazy" class="fs-single-image" src='${new_image}'>`);

    // Update urls
    quickShop.find('a[href=""]').each(function(){
      $(this).attr('href',`/products/${data.handle}`);
    })

    // Update reviews 
    quickShop.find('.quick-view-reviews-wrapper').html(container.find('[data-oke-star-rating]').html());
    // Update title
    quickShop.find('.quick-view-dialog-title').html(data.title);

    // Update price
    quickShop.find('.quick-view-price-wrapper').html("$" + data.price/100);

    $('.js-quick-shop .product_form').html('<svg height="30" width="30" style="animation-duration: 550ms;" class="svelte-spinner fs-result-page-n2keu2" viewBox="0 0 32 32"><circle role="presentation" cx="16" cy="16" r="10" stroke="black" fill="none" stroke-width="2" stroke-dasharray="47.12388980384689,100" stroke-linecap="round"></circle></svg>');
